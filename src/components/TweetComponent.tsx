...
function TweetItem({ createdAt, retweets, likes, comments, numberShare, content, userId, image }: Tweet) {
    ...
    const [isLiked, setIsLiked] = useState<boolean>(false); // État pour les likes
    const [likesCount, setLikesCount] = useState<number>(likes); // Compteur de likes
    ...
    // Gestion du clic sur l'icône des likes
    const handleLikeClick = () => {
        setIsLiked(!isLiked);
        setLikesCount(isLiked ? likesCount - 1 : likesCount + 1); // Incrémente ou décrémente le compteur
    };
                    ...
                    <div className="flex flex-col gap-1 pr-5">
                        <div className="flex flex-row justify-between items-center">
                            <div className="flex items-center flex-wrap gap-1">
                                ...
                                {(user?.certified) && <svg fill="currentColor" className="text-blue-500 w-4" viewBox="0 0 22 22" aria-label="Compte certifié" role="img" data-testid="icon-verified">...</svg>}
                                <p className="text-gray-600 text-[0.91rem]">@{user?.username} . {formatDate(createdAt)}</p>
                            </div>
                            <img className="w-4" src={moreIcon} alt="more Icon" />
                        </div>
                        <p>{content}</p>
                        {image && <img className="w-full border rounded-3xl" src={image} alt="Tweet image" />}
                        <div className="flex justify-between p-1 ">
                            {/* comments icon */}
                            <div className="flex items-center cursor-pointer hover:bg-gray-600 hover:text-blue-300 text-gray-600 px-2 rounded-full">
                                <div className="w-8 h-8 p-2 rounded-full hover:bg-gray-600 hover:bg-opacity-30">
                            ...
                            {/* likes icon */}
                            <div className={`flex items-center cursor-pointer hover:bg-gray-600 hover:text-blue-300 text-gray-600 px-2 rounded-full ${isLiked && "text-red-600"}`} onClick={handleLikeClick}>
                                <div className="w-8 h-8 p-2 rounded-full hover:bg-gray-600 hover:bg-opacity-30">
                                    <svg width="16" height="16" viewBox="0 0 22 20" fill={isLiked ? "red" : "currentColor"} xmlns="http://www.w3.org/2000/svg">
                            ...
                            {/* shares icon */}
                            <div className="flex items-center cursor-pointer hover:bg-gray-600 hover:text-blue-300 text-gray-600 px-2 rounded-full">
                                <div className="w-8 h-8 p-2 hover:bg-opacity-30">
                                    <svg width="16" height="16" viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <p className="text-sm">{numberShare}</p>
                            </div>
                        </div>
                    </div>
                </div>
