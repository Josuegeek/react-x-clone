...
function MiniProfileCard({ profile, name, username, certified, link, followLink }: profileInfo) {
    return (
        <a href={link} className="flex flex-row p-3 justify-between gap-2 items-center cursor-pointer hover:bg-gray-600 hover:bg-opacity-50 rounded-full transition-all duration-200">
            <div className="flex flex-row gap-2 items-center">
                <Profile profile={profile}></Profile>
                <div className="flex flex-col">
                    <p className="flex gap-1 font-bold">{name} {(certified)? <svg fill="currentColor" className="text-blue-500 w-4" viewBox="0 0 22 22" aria-label="Compte certifié" role="img" 
                         ...
                         1.68-.907.46-.46.776-1.044.908-1.681s.075-1.299-.165-1.903c.586-.274 1.084-.705 1.439-1.246.354-.54.551-1.17.569-1.816zM9.662 14.85l-3.429-3.428 1.293-1.302 2.072 2.072 4.4-4.794 1.347 1.246z"></path></g></svg> : "iswa"} </p>
                    <p className="text-opacity-65">{"@"+username}</p>
                </div>
            </div>
            
            {(followLink)? <Button text="Follow" color="white"></Button>:<img className="w-4" src={moreIcon} alt="more Icon" />}
