...
function MiniProfileCard({ profile, name, username, certified, link, followLink }: profileInfo) {
    ...
    let profileNamesClass="flex flex-col",padding="p-3", profileClass=""
    ...
    return (
        <a href={link} className={`flex flex-row ${padding} ${profileClass} justify-between gap-2 items-center cursor-pointer hover:bg-gray-600 hover:bg-opacity-50 rounded-full transition-all duration-200`}>
            ...
            {(followLink)? <Button text="Follow" color="white"></Button>:<img className={"w-4 "+profileNamesClass} src={moreIcon} alt="more Icon" />}
